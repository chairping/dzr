...
class BaseController extends \Think\Controller {
        ...
        * */
        protected function _pageShow($total, $size, $type = 'default') {
                $page = new \Think\Page($total, $size);// 实例化分页类 传入总记录数和每页显示的记录数
                switch($type) {
...
//                        $page->setConfig('theme', '<div class="dataTables_paginate paging_bootstrap_full"><ul class="pagination"> <li>%UP_PAGE%</li> <li>%FIRST%</li> <li>%LINK_PAGE%</li> <li>%END%</li> <li>%DOWN_PAGE%</li> </ul> <ul class="pagination pull-right"> <li>'.$this->_pageNumberWidget($size, C('SEARCH_DEFAULT_PAGE_ASSEMBLAGE')).'</li> <li><i>条/页</i></li> <li><i>(%HEADER%)</i></li> </ul> </div>');
//                        break;
                    default:
                        $theme = <<<EOF
<div class="row">
    ...
    <div class="col-sm-6"><div class="dataTables_paginate paging_bootstrap" id="sample-table_paginate">
        <ul class="pagination">
            <li class="prev disabled"><a href="#">%FIRST% 页</a></li>
            <li class="active">%LINK_PAGE%</li>
            <li class="next disabled"><a href="#">%DOWN_PAGE%页</a></li>
        </ul>
    </div>
</div>
EOF;
